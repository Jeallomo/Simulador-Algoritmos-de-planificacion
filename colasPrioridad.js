...
Prioridad     Algoritmo   Tiempo (Probabilidad)    Promoción
alta          fcfs          50%	                   no aplica
media         srtf          30%                        15
baja          sjf           20%                        10
...
function crearProceso(instLleg, tEje, inibloq, duraBloq){
    procesos.push({instLlegada: instLleg, tEjecucion: tEje,tEjecucionAbsoluto: tEje, tEsperaCola: 0 ,inicioBloq: inibloq, duraBloq:duraBloq, estadoProc:0, tEspera: 0, idProc: procesos.length, tRespuesta: -1, tFinal: 0});
...
//----------------------------------FUNCIONES QUE ORGANIZAN LISTAS--------------------
//organiza los procesos según su tiempo de llegada relativo 
...
//organiza procesos según tiempo de ejecución
function organizarProcesosEjecucion(proc){
    proc.sort(function(a, b) {
        if(a.tEjecucion!=b.tEjecucion){
            return a.tEjecucion-b.tEjecucion;
        }else{
...
function encendido(){
    ...
    while (numProc != procEstadis.length ){      
	    ...
	    //variable que dicta cual proceso de qué lista se va a elegir
        var numAlea = getRandomList();
        console.log(numAlea);
        if( numAlea>=0){
            if ( cpu.proceso == null){
                // cambia su estado a ejecución al primero que es el que se elige
                procesosParaEjecucion[numAlea][0].estadoProc = 1;
                //le pasa el proceso a la cpu
                cpu.setProceso(procesosParaEjecucion[numAlea][0]);
                // se quita de la lista el proceso que paso a la CPU
                procesosParaEjecucion[numAlea].shift();
...
//HECHO
//aumenta la duración de espera de los procesos que ya llegaron para ejecutarse en cada cola
function aumentarTiempoEspera(){
       //recorre la lista de prioridades
    for(let i = 0 ; i< procesosParaEjecucion.length; i++){
        for (let j = 0; j < procesosParaEjecucion[i].length; j++) {
            // le aumenta en 1 su tiempo de espera total
            procesosParaEjecucion[i][j].tEspera++;
            //aumenta su tiempo de espera en la cola actual
            procesosParaEjecucion[i][j].tEsperaCola++;
            //pone su estado en espera
            procesosParaEjecucion[i][j].estadoProc = 3;             
        }      
    }  
}
...
//HECHO
//aumenta la duración de espera de los procesos que ya llegaron para ejecutarse en cada cola
function promoverProceso(){
    //recorre la lista de prioridades, no se toma en cuenta la cola de prioridad alta
    for(let i = 1 ; i< procesosParaEjecucion.length; i++){
        let cantProcesoLista = procesosParaEjecucion[i].length;
        for (let j = 0; j < cantProcesoLista; j++) {
            if(i ==1 && procesosParaEjecucion[i][j].tEsperaCola >= 10){
                //reinicia su contador de espera
                procesosParaEjecucion[i][j].tEsperaCola=0;
                //sube una divisón el proceso 
                procesosParaEjecucion[i--].unshift(procesosParaEjecucion[i][j]);
                //lo elimina de la cola actual
                procesosParaEjecucion[i].splice(i,1);
                cantProcesoLista--;
                j--;
            }
            if(i ==2 && procesosParaEjecucion[i][j].tEsperaCola >= 5){
                //reinicia su contador de espera
                procesosParaEjecucion[i][j].tEsperaCola=0;
                //sube una divisón el proceso 
                procesosParaEjecucion[i--].unshift(procesosParaEjecucion[i][j]);
     ...
     //organiza la cola de procesos de media prioridad en orden de ejecución variable
    organizarProcesosEjecucion(procesosParaEjecucion[1]);
    //organiza los procesos de baja prioridad según su tiempo de ejecución absoluta
    organizarProcesosEjecucionAbsoluta(procesosParaEjecucion[2]);
    //organiza los procesos de alta prioridad según su orden de llegada
    organizarProcesosLlegada(procesosParaEjecucion[0]);
}

//HECHO
//revisa los procesos que ya llegaron y los ordena según su tiempo de ejecución
    ...
    if (procesos.length!=0){
        ...
        for(let i =0; i<lonProcSinEx; i++){
            if(procesos[i].instLlegada<=contEje){
                ...
                //pone el proceso en el inicio de la lista de prioridad media
                procesosParaEjecucion[1].unshift(procesos[i]);
                //quita el proceso de la lista de procesos
                procesos.splice(i,1);
                i--;
                lonProcSinEx--;
            }
...
//HECHO
//administra los procesos bloqueados
    ...
    if (procesosBloqueados.length!=0){
        ...
        for(let i =0; i<lonProcBloq; i++){
            ...
            if(procesosBloqueados[i].duraBloq == 0){
                ...
                //pone el proceso desbloqueado en el inicio de la lista de baja prioridad
                procesosParaEjecucion[2].unshift(procesosBloqueados[i]);
...
function getRandomList(){
    var num=Math.random();
    if(procesosParaEjecucion[0].length>0 && procesosParaEjecucion[1].length>0 && procesosParaEjecucion[2].length>0){
        if(num < 0.5) return 0;  //Lista de prioridad alta  P=50%
        else if(num < 0.8) return 1; // Lista de prioridad media   P=30%
        else return 2; //Lista de prioridad baja   P=20%
    }else if(procesosParaEjecucion[0].length>0 && procesosParaEjecucion[1].length>0){
        if(num < 0.6) return 0;  //Lista de prioridad alta  P=60%
        else return 1; // Lista de prioridad media   P=40%
    }else if(procesosParaEjecucion[0].length>0 && procesosParaEjecucion[2].length>0){
        if(num < 0.65) return 0;  //Lista de prioridad alta  P=65%
        else return 2; //Lista de prioridad baja   P=35%
    }else if(procesosParaEjecucion[1].length>0  && procesosParaEjecucion[2].length>0){
        if(num < 0.55) return 1;// Lista de prioridad media   P=55%
        else return 2; //Lista de prioridad baja   P=45%
    }else if(procesosParaEjecucion[2].length>0){
        return 2; //Lista de prioridad baja   
    }else if(procesosParaEjecucion[1].length>0  && procesosParaEjecucion[1].length>0){
        return 1;// Lista de prioridad media   
    }else if( procesosParaEjecucion[0].length>0){
        return 0;// Lista de prioridad alta
    }else return -1;
}
...
function iniciar() {
    ...
    //pone los procesos en las tablas del html
    ponerProcesos(); 
...
function ponerDatosGrafica() {
    //referencia al objeto
    let canvas = document.getElementById("miCanvas").getContext("2d");
    //conjunto de datos

    dataEjecución 
    const gEjecucion = {
        label: 'Procesos en ejecución',
        data: [[3, 12], 19, 3, 5, 2, 3],
